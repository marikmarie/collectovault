...
export default function InvoiceDetailModal({ invoice, onClose }: Props) {
  ...
  // Data mapping based on your API structure
  ...
  // Handle downloading the invoice data as a JSON file
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `Invoice_${details.id || 'Data'}.json`;
      ...
      >
        ...
        <div className="px-6 flex justify-between items-center border-b border-gray-100">
          <div className="flex gap-8">
            ...
            >
              Details
              {tab === 'details' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#9c27b0]" />}
            </button>
            <button 
            ...
            >
              Payments
              {tab === 'payment' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#9c27b0]" />}
            </button>
          </div>
          ...
          <div className="flex gap-2 mb-2">
            <button 
              ...
              className="flex items-center gap-1.5 bg-[#625f63] text-white px-3 py-1.5 rounded text-sm font-bold uppercase shadow-sm hover:bg-gray-700 transition-colors"
            >
          ...
          {tab === 'details' ? (
            <table className="w-full border-collapse">
              <tbody className="text-gray-700">
                <tr className="border-b border-gray-100">
                  <td className="py-3 font-bold text-gray-800 w-1/3 text-sm uppercase tracking-wider">Date</td>
                  <td className="py-3 text-sm">{details.invoice_date}</td>
                </tr>
                <tr className="border-b border-gray-100">
                  <td className="py-3 font-bold text-gray-800 text-sm uppercase tracking-wider">Client</td>
                  <td className="py-3 text-sm">{details.client_name}</td>
                </tr>
                <tr className="border-b border-gray-100">
                  <td className="py-3 font-bold text-gray-800 align-top text-sm uppercase tracking-wider">Services</td>
                  <td className="py-3">
                ...
                <tr className="border-b border-gray-100">
                  <td className="py-3 font-bold text-gray-800 text-sm uppercase tracking-wider">Total</td>
                ...
                <tr className="border-b border-gray-100 text-green-600">
                  <td className="py-3 font-bold text-sm uppercase tracking-wider">Paid</td>
                  <td className="py-3 font-bold text-right">
                    {Number(totalPaid).toLocaleString()}
                  </td>
                </tr>
                <tr className="border-b border-gray-100 text-red-500">
                  <td className="py-3 font-bold text-sm uppercase tracking-wider">Balance</td>
                  <td className="py-3 font-bold text-right text-lg">
                    {Number(amountLess).toLocaleString()}
                  </td>
                </tr>
                <tr>
                  <td className="py-3 font-bold text-gray-800 text-sm uppercase tracking-wider">Issued By</td>
            ...
            <div className="w-full">
                ...
                <tbody className="text-gray-600">
                  {payments.map((pay: any, i: number) => (
                    <tr key={i} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="py-4">
                        <div className="font-bold text-gray-800 text-sm">{pay.payment_option}</div>
                        <div className="text-[10px] text-gray-400 font-mono">{pay.transaction_id}</div>
                      </td>
                      <td className="py-4 text-[13px] italic">
                        By: {pay.user_name}
                      </td>
              ...
              <div className="mt-4 flex justify-between items-center bg-gray-100 p-3 rounded font-bold">
                <span className="uppercase text-xs tracking-widest text-gray-700">Total Paid</span>
                <span className="text-lg">{Number(totalPaid).toLocaleString()}</span>
              </div>
